...
const prompts = `Question: What is the elevation range for the area that the eastern sector of the
 ...
 Based upon the above prompt format, answer the user question in this format only. Doesn't stop in halfway.
 Give Full Response and give complete text
 Question: "",
 {agent_scratchpad}`;
...
async function fetchWikipedia(query) {
  try {
    ...
    return `Could not find information on ${query}.`;
  }
}

async function makeRequest(prompt, input) {
  const handleChunk = async (chunkObj) => {
    if (
      chunkObj &&
      (chunkObj.includes("Thought") ||
        chunkObj.includes("Action") ||
        chunkObj.includes("Observ"))
    ) {
      // If a new section is encountered, display the current section to the user
      console.log(currentSection);
      currentSection = ""; // Reset the current section
    }

    // Check if chunkObj is defined before appending to currentSection and str
    if (chunkObj) {
      currentSection += chunkObj;

      // Check if the chunkObj contains a Wikipedia search query
      const matches = chunkObj.match(/Search\[(.*?)\]/);
      if (matches && matches[1]) {
        const wikipediaText = await fetchWikipedia(matches[1]);
        console.log(`Wikipedia Results for ${matches[1]}: ${wikipediaText}`);
      }

      str = str.concat(chunkObj);
    }
  };
  ...
  for await (const chunk of initialResponse) {
    // Check if chunkObj is defined before accessing properties
    let chunkObj = chunk.choices[0]?.delta?.content;

    await handleChunk(chunkObj);
