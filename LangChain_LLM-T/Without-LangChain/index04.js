...
async function run() {
  try {
    const input = { person: "David Chanoff" };

    const thoughtsAndActions = [
      {
        thought:
          "I need to search David Chanoff and the U.S. Navy admiral, find the ambassador to the United Kingdom, then find the President they served under.",
        action: `Search [${input.person}]`,
      },
      {
        thought:
          "David Chanoff has collaborated with a U.S. Navy admiral. I need to search the U.S. Navy admiral and find the ambassador to the United Kingdom.",
        action: "Search [U.S. Navy admiral]",
      },
      {
        thought:
          "I need to search Admiral William J. Crowe and find the ambassador to the United Kingdom.",
        action: "Search [Admiral William J. Crowe]",
      },
      {
        thought:
          "Admiral William J. Crowe was the ambassador to the United Kingdom under President Bill Clinton, so the answer is Bill Clinton.",
        action: "Finish [Bill Clinton]",
      },
    ];

    const result = await processChain(input, thoughtsAndActions);
    console.log(result);
  } catch (error) {
    console.error(`Error in chain processing: ${error.message}`);
  }
