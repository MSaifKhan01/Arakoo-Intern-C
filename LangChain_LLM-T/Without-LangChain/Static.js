...
const prompts = `Question: What is the elevation range for the area that the eastern sector of the
 ...
 Based upon the above prompt format, answer the user question in this format only. Doesn't stop in halfway.Give Full Response.
 Question: "",
 {agent_scratchpad}`;
...
async function ProcessChain(input) {
  let response = await generateResponse(prompts, input);
  return response;
}
